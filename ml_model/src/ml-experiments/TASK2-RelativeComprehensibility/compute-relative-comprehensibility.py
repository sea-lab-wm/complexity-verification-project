...
csv_data_dict = {
	...
	# "developer_position_x": 0,
	# "developer_position_y": 0,
	# "PE gen_x": 0,
	# "PE gen_y": 0,
	# "PE spec (java)_x": 0,
	# "PE spec (java)_y": 0
...
def csv_write_header(output_file, csv_data_dict):
	with open(configs.ROOT_PATH + "/data/" + output_file, "w+") as csv_file:
...
# 	plt.savefig(configs.ROOT_PATH + "/data/relative_comprehensibility_diff_boxplot.png")
...
def generate_ds(output_file, epsilon, dynamic_epsilon):
	for index, s1 in enumerate(snippet_methods):
			for target in binary_targets:
				...
				for s2 in np.delete(snippet_methods, index):
					...
					if target == "PBU" or target == "ABU" or target == "ABU50" or target == "AU":
					...
					# csv_data_dict["developer_position_x"] = math.floor(method_df_s1["developer_position"].mean())
					# csv_data_dict["developer_position_y"] = math.floor(method_df_s2["developer_position"].mean())
					# csv_data_dict["PE gen_x"] = math.floor(method_df_s1["PE gen"].mean())
					# csv_data_dict["PE gen_y"] = math.floor(method_df_s2["PE gen"].mean())
					# csv_data_dict["PE spec (java)_x"] = math.floor(method_df_s1["PE spec (java)"].mean())
					# csv_data_dict["PE spec (java)_y"] = math.floor(method_df_s2["PE spec (java)"].mean())


					dict_to_csv(configs.ROOT_PATH + "/data/" + output_file, csv_data_dict)
	...
	for index, s1 in enumerate(snippet_methods):
		for target in binary_targets:
			...
			csv_data_dict["dynamic_epsilon"] = dynamic_epsilon
			# csv_data_dict["developer_position_x"] = math.floor(method_df_s1["developer_position"].mean())
			# csv_data_dict["developer_position_y"] = math.floor(method_df_s1["developer_position"].mean())
			# csv_data_dict["PE gen_x"] = math.floor(method_df_s1["PE gen"].mean())
			# csv_data_dict["PE gen_y"] = math.floor(method_df_s1["PE gen"].mean())
			# csv_data_dict["PE spec (java)_x"] = math.floor(method_df_s1["PE spec (java)"].mean())
			# csv_data_dict["PE spec (java)_y"] = math.floor(method_df_s1["PE spec (java)"].mean())
...
def main():
	...
	output_file = "DS6_relative_comprehensibility_e_0_static.csv"
	csv_write_header(output_file, csv_data_dict)
	generate_ds(output_file, 0, False)

	output_file = "DS6_relative_comprehensibility_e_dynamic.csv"
	csv_write_header(output_file, csv_data_dict)
	generate_ds(output_file, 0, True)

	## run the add_code_features.py
	add_code_features("DS6_relative_comprehensibility_e_dynamic.csv", "DS6_train_date_with_warnings_e_dynamic.csv")
	add_code_features("DS6_relative_comprehensibility_e_0_static.csv", "DS6_train_date_with_warnings_e_0_static.csv")

	## read the train data and merge to single file
	train_df_static = pd.read_csv(configs.ROOT_PATH + "data/DS6_train_date_with_warnings_e_dynamic.csv")
	train_df_dynamic = pd.read_csv(configs.ROOT_PATH + "data/DS6_train_date_with_warnings_e_0_static.csv")
	complete_df = pd.concat([train_df_static, train_df_dynamic], axis=0)
	complete_df.to_csv(configs.ROOT_PATH + "data/merged_ds6.csv", index=False)
