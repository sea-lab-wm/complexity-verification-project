...
class FeatureSelection:
    ...
    def compute_mutual_information(self):
        ...
        # plt.savefig(configs.RELATIVE_COMPREHENSIBILITY_ROOT_PATH + '/featureselection/mutual_info_' + self.code_comprehension_target + '_' +  self.y.columns[0] + '_' + 'dynamic_epsilon_' + str(self.dynamic_epsilon) +'.png')

        ## save the mutual information to a csv file
        with open(configs.RELATIVE_COMPREHENSIBILITY_ROOT_PATH + '/' + configs.MI_OUTPUT_FILE_NAME, "a") as csv_file:
            for feature in self.X.columns: 
                csv_file.write(feature + "," + self.code_comprehension_target + ',' + str(self.dynamic_epsilon) +',' + str(mi_dict[feature]) + '\n')
    
       
        mi = pd.read_csv(configs.RELATIVE_COMPREHENSIBILITY_ROOT_PATH + '/' + configs.MI_OUTPUT_FILE_NAME)
    ...
    def compute_kendals_tau(self):
        ...
        for feature in self.X.columns:
            ...
            with open(configs.RELATIVE_COMPREHENSIBILITY_ROOT_PATH + '/' + configs.KENDALS_OUTPUT_FILE_NAME, "a") as csv_file:
        ...
        kendall = pd.read_csv(configs.RELATIVE_COMPREHENSIBILITY_ROOT_PATH + '/' + configs.KENDALS_OUTPUT_FILE_NAME)
