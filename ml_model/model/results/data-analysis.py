...
### RQ 1 Answer - which Feature set (with SMOTE or not) is better for which target? ###
# for feature_set in feature_sets:
#     for smote_value in smote:
#         for target in targets:

#             ## filter overall results according feature set, warning feature, target, smote value and metric
#             filtered_df = df.query("feature_set == '" + feature_set + "' and target == '" + target + "' and use_smote == " + str(smote_value))
                    
#             ## filtered by F1 metric
#             filtered_df_f1 = filtered_df.query("metric == 'f1'").dropna()

#             ## get only the values where diff(mean) > 0
#             filtered_df_f1_mean_improved = filtered_df_f1[filtered_df_f1["diff(mean)"] > 0]
#             ## avg mean improvement
#             avg_mean_f1_improvement = filtered_df_f1_mean_improved["diff(mean)"].mean()

#             ## get only the values where diff(median) > 0
#             filtered_df_f1_median_improved = filtered_df_f1[filtered_df_f1["diff(median)"] > 0]
#             ## avg median improvement
#             avg_median_f1_improvement = filtered_df_f1_median_improved["diff(median)"].mean()

#             ## get only the values where diff(median) > 0 and diff(mean) > 0
#             filtered_df_f1_mean_median_improved = filtered_df_f1[(filtered_df_f1["diff(median)"] > 0) & (filtered_df_f1["diff(mean)"] > 0)]
#             ## avg median improvement
#             avg_mean_median_f1_improvement = filtered_df_f1_mean_median_improved["diff(mean)"].mean()
#             avg_median_median_f1_improvement = filtered_df_f1_mean_median_improved["diff(median)"].mean()

#             ## BOX PLOT FOR F1 IMPROVEMENT ##
#             plt.figure(figsize=(15, 6))
#             ## number of data points
#             n1 = filtered_df_f1_mean_improved["diff(mean)"].shape[0]
#             n2 = filtered_df_f1_median_improved["diff(median)"].shape[0]
#             n3 = filtered_df_f1_mean_median_improved["diff(median)"].shape[0] ## since both mean and median are improved, we can take any one of the columns
                
                
#             plt.boxplot([filtered_df_f1_mean_improved["diff(mean)"], filtered_df_f1_median_improved["diff(median)"], filtered_df_f1_mean_median_improved["diff(mean)"], filtered_df_f1_mean_median_improved["diff(median)"]], labels=["Mean(diff F1-only Mean improved)", "Median (diff F1-only Median improved)", "Mean (diff F1-both Mean and Median improved)", "Median (diff F1-both Mean and Median improved)"])
#             plt.title(feature_set + " | " + target + " | SMOTE = " + str(smote_value) + " | #data points (mean): " + str(n1) + " | #data points (median): " + str(n2) + " | #data points (mean+median): " + str(n3) )
...
#             plt.scatter(1, avg_median_f1_improvement, marker='o', color='red', s=10)
#             plt.scatter(2, avg_mean_f1_improvement, marker='o', color='red', s=10)
#             plt.scatter(3, avg_mean_median_f1_improvement, marker='o', color='red', s=10)
#             plt.scatter(4, avg_median_median_f1_improvement, marker='o', color='red', s=10)
#             legend_elements = [Line2D([0], [0], marker='o', color='w', label='mean', markerfacecolor='r', markersize=10)]
#             plt.legend(handles=legend_elements, loc='upper right')

#             ## create final-boxplots folder if it doesn't exist in results directory
#             if not os.path.exists(ROOT_PATH + 'results/all-results/feature-set-wise-result-distribution/' + feature_set + "-" + target + "-" + str(smote_value)):
#                 os.makedirs(ROOT_PATH + 'results/all-results/feature-set-wise-result-distribution/' + feature_set + "-" + target + "-" + str(smote_value))
#             plt.savefig(ROOT_PATH + 'results/all-results/feature-set-wise-result-distribution/' + feature_set + "-" + target + "-" + str(smote_value) + '/f1-improvement.png')
#             plt.clf()
#             plt.close() 



## RQ 2 Answer - which warning feature is better  with which feature set for overall all targets? ##
# for feature_set in feature_sets:
#     for warning_feature in warning_features:
#         for smote_value in smote:
                
#                 ## filter overall results according feature set, warning feature, target, smote value and metric
#                 filtered_df = df.query("warning_feature == '" + warning_feature + "' and feature_set == '" + feature_set + "' and use_smote == " + str(smote_value))
...
#                 n1 = filtered_df_f1_mean_improved["diff(mean)"].shape[0]
#                 n2 = filtered_df_f1_median_improved["diff(median)"].shape[0]
#                 n3 = filtered_df_f1_mean_median_improved["diff(median)"].shape[0]

#                 plt.boxplot([filtered_df_f1_mean_improved["diff(mean)"], filtered_df_f1_median_improved["diff(median)"], filtered_df_f1_mean_median_improved["diff(mean)"], filtered_df_f1_mean_median_improved["diff(median)"]], labels=["Mean(diff F1-only Mean improved)", "Median (diff F1-only Median improved)", "Mean (diff F1-both Mean and Median improved)", "Median (diff F1-both Mean and Median improved)"])

#                 plt.title(feature_set + " | " + warning_feature + " | SMOTE = " + str(smote_value) + " | #data points (mean): " + str(n1) + " | #data points (median): " + str(n2) + " | #data points (mean+median): " + str(n3) )
...
#                 ## create final-boxplots folder if it doesn't exist in results directory
#                 if not os.path.exists(ROOT_PATH + 'results/all-results/warning-feature-wise-result-distribution/' + feature_set + "-" + warning_feature + "-" + str(smote_value)):
#                     os.makedirs(ROOT_PATH + 'results/all-results/warning-feature-wise-result-distribution/' + feature_set + "-" + warning_feature + "-" + str(smote_value))

#                 plt.savefig(ROOT_PATH + 'results/all-results/warning-feature-wise-result-distribution/' + feature_set + "-" + warning_feature + "-" + str(smote_value) + '/f1-improvement.png')
#                 plt.clf()
#                 plt.close()

## Which Feature is better for which warning feature? ##
for target in targets:
    for smote_value in smote:
         ## filter overall results according feature set, warning feature, target, smote value and metric
        filtered_df = df.query("target == '" + target + "' and use_smote == " + str(smote_value))
        ...
        plt.title(target + " | SMOTE = " + str(smote_value) + " | #data points (mean): " + str(n1) + " | #data points (median): " + str(n2) + " | #data points (mean+median): " + str(n3) )
        ...
        ## create final-boxplots folder if it doesn't exist in results directory
        if not os.path.exists(ROOT_PATH + 'results/all-results/target-wise-result-distribution/' + target + "-" + str(smote_value)):
            os.makedirs(ROOT_PATH + 'results/all-results/target-wise-result-distribution/' + target + "-" + str(smote_value))


        plt.savefig(ROOT_PATH + 'results/all-results/target-wise-result-distribution/' + target + "-" + str(smote_value) + '/f1-improvement.png')
        plt.clf()
        plt.close()    
