...
############################
## filter overall results
overall_df = df.query("iteration == 'overall' and use_smote == " + str(use_SMOTE))
...
overall_df.to_csv(ROOT_PATH + 'results/overall_results_'+ str(SMOTE_TAG) + '.csv', index=False)
...
best_code_features.to_csv(ROOT_PATH + 'results/best_code_models_'+ str(SMOTE_TAG) + '.csv', index=False)
best_code_warning_features.to_csv(ROOT_PATH + 'results/best_code+warning_models_'+ str(SMOTE_TAG) + '.csv', index=False)
...
## write header
with open(ROOT_PATH + 'results/mean_median_'+ str(SMOTE_TAG) + '.csv', "w") as csv_file:
...
for model_name in models:
    for target in targets:
        ...
        for metric in metrics.keys():
            ...
            plt.savefig(ROOT_PATH + 'results/final-boxplots/' + target + '_' + model_name + '_' + metric +'_boxplot_'+ str(SMOTE_TAG) + '.png')
            plt.close() 

        with open(ROOT_PATH + 'results/mean_median_'+ str(SMOTE_TAG) + '.csv', 'a') as csv_file:
            writer = csv.DictWriter(csv_file, fieldnames=header.keys())
...
for column in columns:
    ...
    plt.savefig(ROOT_PATH + 'results/histograms/final_hist_' + column +'_'+ str(SMOTE_TAG) + '.png')
...
with open(ROOT_PATH + "classification/experiments.jsonl") as jsonl_file:
    ...
    for experiment in experiments:
        ...
        plt.suptitle('')
        plt.tight_layout()
        plt.savefig(ROOT_PATH + 'results/histograms/final_hist_code_features'+ experiment["target"] +'_'+ str(SMOTE_TAG) + '.png')
        plt.close()
...
## write header
with open(ROOT_PATH + 'results/final_statistics_'+ str(SMOTE_TAG) + '.csv', "w") as csv_file:
...
for model_name in models:
    for target in targets:
        ...
        for metric in metrics.keys():
            ...
            with open(ROOT_PATH + 'results/final_statistics_'+ str(SMOTE_TAG) + '.csv', 'a') as csv_file:
                writer = csv.DictWriter(csv_file, fieldnames=tests.keys())
