'''
This file is used to generate the experiments for regression. 
'''
import json
import os
...
def generate_experiments(experiment_id, target, feature_set, features):
    experiment = {
        "experiment_id": "exp" + str(experiment_id) + "-"+ target + "-" + feature_set, 
        "features": features, 
        "target": target, 
    }
    return experiment
...
with open(ROOT_PATH + 'regression/experiments.jsonl', 'w+') as f:
    f.write('')

experiment_id = 0
for warning_feature in warning_features:
    for target in regression_targets:
        with open(ROOT_PATH + 'regression/experiments.jsonl', 'a') as f:
                
            experiment_id = experiment_id+1
            jsondump =json.dumps(generate_experiments(experiment_id, target, 'feature-set1', [warning_feature] + feature_set1[target]))
            f.write(jsondump)
            f.write('\n')

            experiment_id = experiment_id+1
            jsondump = json.dumps(generate_experiments(experiment_id, target, 'feature-set2', [warning_feature] + feature_set2[target]))
            f.write(jsondump)
            f.write('\n')

            experiment_id = experiment_id+1
            jsondump = json.dumps(generate_experiments(experiment_id, target, 'feature-set3', [warning_feature] + set_3_4['feature-set3'][target]))
            f.write(jsondump)
            f.write('\n')

            experiment_id = experiment_id+1
            jsondump = json.dumps(generate_experiments(experiment_id, target, 'feature-set4', [warning_feature] + set_3_4['feature-set4'][target]))
            f.write(jsondump)
            f.write('\n')
                
## remove the last new line character
with open(ROOT_PATH + 'regression/experiments.jsonl', 'rb+') as f:
