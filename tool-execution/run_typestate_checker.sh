...
# the classpath and supporting variables for dataset 63
DATASET63=$(./gradlew :dataset63:printClasspath -q)
# these two variables are not really necessary, but makes the command line a little easier to read
CF314PATH=$(realpath checker-framework-3.14.0/checker/dist/checker.jar)
OUTFILE63=$(realpath data/typestate_checker_output_dataset_63.txt)

java -jar ${CF314PATH} -classpath jatyc.jar:"${DATASET63}" -processor jatyc.JavaTypestateChecker -d classes -Awarns -Xmaxwarns 10000 $(find dataset63/src/main/java -name "*.java") 2> ${OUTFILE63}


# the classpath and supporting variables for dataset 8
DATASET8=$(./gradlew :dataset8:printClasspath -q)
# these two variables are not really necessary, but makes the command line a little easier to read
CF314PATH=$(realpath checker-framework-3.14.0/checker/dist/checker.jar)
OUTFILE8=$(realpath data/typestate_checker_output_dataset_8.txt)

java -jar ${CF314PATH} -classpath jatyc.jar:"${DATASET8}" -processor jatyc.JavaTypestateChecker -d classes -Awarns -Xmaxwarns 10000 $(find dataset8/src/main/java -name "*.java") 2> ${OUTFILE8}
